...
/// struct for typed successes of method [`create_user_defined_message`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateUserDefinedMessageSuccess {
    Status201(models::ApiPeriodV2010PeriodAccountPeriodCallPeriodUserDefinedMessage),
...
) -> Result<ResponseContent<CreateUserDefinedMessageSuccess>, Error<CreateUserDefinedMessageError>>
{
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<CreateUserDefinedMessageSuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
