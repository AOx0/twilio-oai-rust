...
pub enum FetchMemberSuccess {
    Status200(models::ApiPeriodV2010PeriodAccountPeriodQueuePeriodMember),
...
pub enum ListMemberSuccess {
    Status200(models::ListMemberResponse),
    UnknownValue(serde_json::Value),
}
...
pub async fn fetch_member(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<FetchMemberSuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
...
pub async fn list_member(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<ListMemberSuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
...
pub async fn update_member(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<UpdateMemberSuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
