...
pub enum ListIncomingPhoneNumberMobileSuccess {
    Status200(models::ListIncomingPhoneNumberMobileResponse),
    UnknownValue(serde_json::Value),
}
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<CreateIncomingPhoneNumberMobileSuccess> =
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<ListIncomingPhoneNumberMobileSuccess> =
            serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
