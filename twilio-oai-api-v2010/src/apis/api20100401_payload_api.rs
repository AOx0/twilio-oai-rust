...
pub enum FetchRecordingAddOnResultPayloadSuccess {
    Status200(models::ApiPeriodV2010PeriodAccountPeriodRecordingPeriodRecordingAddOnResultPeriodRecordingAddOnResultPayload),
    UnknownValue(serde_json::Value),
}
...
pub enum ListRecordingAddOnResultPayloadSuccess {
    Status200(models::ListRecordingAddOnResultPayloadResponse),
    UnknownValue(serde_json::Value),
}
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<DeleteRecordingAddOnResultPayloadSuccess> =
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<FetchRecordingAddOnResultPayloadSuccess> =
            serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<ListRecordingAddOnResultPayloadSuccess> =
            serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
