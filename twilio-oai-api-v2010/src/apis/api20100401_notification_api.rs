...
pub enum FetchNotificationSuccess {
    Status200(models::ApiPeriodV2010PeriodAccountPeriodNotificationInstance),
...
pub enum ListNotificationSuccess {
    Status200(models::ListNotificationResponse),
    UnknownValue(serde_json::Value),
}
...
pub async fn fetch_notification(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<FetchNotificationSuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
...
pub async fn list_notification(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<ListNotificationSuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
