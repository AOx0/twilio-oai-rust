...
pub enum FetchMediaSuccess {
    Status200(models::ApiPeriodV2010PeriodAccountPeriodMessagePeriodMedia),
    UnknownValue(serde_json::Value),
}
...
pub async fn delete_media(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<DeleteMediaSuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
...
pub async fn fetch_media(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<FetchMediaSuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
