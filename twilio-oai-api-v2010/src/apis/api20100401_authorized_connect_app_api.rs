...
pub enum FetchAuthorizedConnectAppSuccess {
    Status200(models::ApiPeriodV2010PeriodAccountPeriodAuthorizedConnectApp),
...
pub enum ListAuthorizedConnectAppSuccess {
    Status200(models::ListAuthorizedConnectAppResponse),
    UnknownValue(serde_json::Value),
}
...
) -> Result<ResponseContent<FetchAuthorizedConnectAppSuccess>, Error<FetchAuthorizedConnectAppError>>
{
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<FetchAuthorizedConnectAppSuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
...
) -> Result<ResponseContent<ListAuthorizedConnectAppSuccess>, Error<ListAuthorizedConnectAppError>>
{
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<ListAuthorizedConnectAppSuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
