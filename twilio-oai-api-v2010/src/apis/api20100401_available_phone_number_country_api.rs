...
pub enum FetchAvailablePhoneNumberCountrySuccess {
    Status200(models::ApiPeriodV2010PeriodAccountPeriodAvailablePhoneNumberCountry),
...
pub enum ListAvailablePhoneNumberCountrySuccess {
    Status200(models::ListAvailablePhoneNumberCountryResponse),
    UnknownValue(serde_json::Value),
}
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<FetchAvailablePhoneNumberCountrySuccess> =
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<ListAvailablePhoneNumberCountrySuccess> =
            serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
