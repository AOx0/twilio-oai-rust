...
pub enum FetchKeySuccess {
    Status200(models::ApiPeriodV2010PeriodAccountPeriodKey),
    UnknownValue(serde_json::Value),
}
...
pub enum ListKeySuccess {
    Status200(models::ListKeyResponse),
    UnknownValue(serde_json::Value),
}
...
pub async fn delete_key(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<DeleteKeySuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
...
pub async fn fetch_key(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<FetchKeySuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
...
pub async fn list_key(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<ListKeySuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
            entity,
