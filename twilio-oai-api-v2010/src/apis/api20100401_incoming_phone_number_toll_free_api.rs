...
pub enum CreateIncomingPhoneNumberTollFreeSuccess {
    Status201(models::ApiPeriodV2010PeriodAccountPeriodIncomingPhoneNumberPeriodIncomingPhoneNumberTollFree),
...
pub enum ListIncomingPhoneNumberTollFreeSuccess {
    Status200(models::ListIncomingPhoneNumberTollFreeResponse),
    UnknownValue(serde_json::Value),
}
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<CreateIncomingPhoneNumberTollFreeSuccess> =
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<ListIncomingPhoneNumberTollFreeSuccess> =
            serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            status,
            content,
